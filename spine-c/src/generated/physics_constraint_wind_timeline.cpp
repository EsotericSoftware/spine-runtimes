...
spine_physics_constraint_wind_timeline spine_physics_constraint_wind_timeline_create(size_t frameCount, size_t bezierCount,
	...
	return (spine_physics_constraint_wind_timeline) new (__FILE__, __LINE__)
		PhysicsConstraintWindTimeline(frameCount, bezierCount, physicsConstraintIndex);
}
...
float spine_physics_constraint_wind_timeline_before_first_key(spine_mix_from from, float alpha, float current, float setup) {
	return PhysicsConstraintWindTimeline::beforeFirstKey((MixFrom) from, alpha, current, setup);
}
