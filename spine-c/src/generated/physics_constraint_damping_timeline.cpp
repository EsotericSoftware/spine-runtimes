...
float spine_physics_constraint_damping_timeline_before_first_key(spine_mix_from from, float alpha, float current, float setup) {
	return PhysicsConstraintDampingTimeline::beforeFirstKey((MixFrom) from, alpha, current, setup);
}
