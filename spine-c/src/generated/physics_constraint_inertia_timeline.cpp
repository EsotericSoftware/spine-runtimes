...
spine_physics_constraint_inertia_timeline spine_physics_constraint_inertia_timeline_create(size_t frameCount, size_t bezierCount,
																						   int physicsConstraintIndex) {
	return (spine_physics_constraint_inertia_timeline) new (__FILE__, __LINE__)
		PhysicsConstraintInertiaTimeline(frameCount, bezierCount, physicsConstraintIndex);
}
...
float spine_physics_constraint_inertia_timeline_before_first_key(spine_mix_from from, float alpha, float current, float setup) {
	return PhysicsConstraintInertiaTimeline::beforeFirstKey((MixFrom) from, alpha, current, setup);
}
