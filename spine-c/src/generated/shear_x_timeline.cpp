...
float spine_shear_x_timeline_get_relative_value(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, float current,
												float setup) {
	ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getRelativeValue(time, alpha, (MixFrom) from, add, current, setup);
}

float spine_shear_x_timeline_get_absolute_value_1(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, float current,
												  float setup) {
	ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getAbsoluteValue(time, alpha, (MixFrom) from, add, current, setup);
}

float spine_shear_x_timeline_get_absolute_value_2(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, float current,
												  float setup, float value) {
	ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getAbsoluteValue(time, alpha, (MixFrom) from, add, current, setup, value);
}

float spine_shear_x_timeline_get_scale_value(spine_shear_x_timeline self, float time, float alpha, spine_mix_from from, bool add, bool out,
											 float current, float setup) {
	ShearXTimeline *_self = (ShearXTimeline *) self;
	return _self->getScaleValue(time, alpha, (MixFrom) from, add, out, current, setup);
}

float spine_shear_x_timeline_before_first_key(spine_mix_from from, float alpha, float current, float setup) {
	return ShearXTimeline::beforeFirstKey((MixFrom) from, alpha, current, setup);
}
