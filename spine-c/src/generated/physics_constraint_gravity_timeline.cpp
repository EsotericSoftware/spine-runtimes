...
spine_physics_constraint_gravity_timeline spine_physics_constraint_gravity_timeline_create(size_t frameCount, size_t bezierCount,
	...
	return (spine_physics_constraint_gravity_timeline) new (__FILE__, __LINE__)
		PhysicsConstraintGravityTimeline(frameCount, bezierCount, physicsConstraintIndex);
}
...
float spine_physics_constraint_gravity_timeline_before_first_key(spine_mix_from from, float alpha, float current, float setup) {
	return PhysicsConstraintGravityTimeline::beforeFirstKey((MixFrom) from, alpha, current, setup);
}
