...
(function (spine) {
    ...
    (function (threejs) {
        var MeshBatcher = (function (_super) {
            ...
            MeshBatcher.prototype.addMaterialGroup = function (indicesLength, materialGroup) {
                var currentGroup = this.materialGroups[this.materialGroups.length - 1];
            ...
            MeshBatcher.prototype.closeMaterialGroups = function () {
                var geometry = this.geometry;
                for (var i = 0; i < this.materialGroups.length; i++) {
                    var _a = this.materialGroups[i], startIndex = _a[0], count = _a[1], materialGroup = _a[2];
                    geometry.addGroup(startIndex, count, materialGroup);
                }
            };
            MeshBatcher.prototype.findMaterialGroup = function (slotTexture, slotBlendMode) {
                var blending = threejs.ThreeJsTexture.toThreeJsBlending(slotBlendMode);
                var group = -1;
                if (Array.isArray(this.material)) {
                    for (var i = 0; i < this.material.length; i++) {
                        var meshMaterial_1 = this.material[i];
                        if (meshMaterial_1.uniforms.map.value === null) {
                            meshMaterial_1.uniforms.map.value = slotTexture;
                            meshMaterial_1.blending = blending;
                            meshMaterial_1.needsUpdate = true;
                            return i;
                        }
                        if (meshMaterial_1.uniforms.map.value === slotTexture && meshMaterial_1.blending === blending) {
                            return i;
                        }
                    }
                    var meshMaterial = new threejs.SkeletonMeshMaterial();
            ...
            SkeletonMesh.prototype.updateGeometry = function () {
                ...
                for (var i = 0, n = drawOrder.length; i < n; i++) {
                    ...
                    if (texture != null) {
                        ...
                        var slotTexture = texture.texture;
                        var materialGroup = batch.findMaterialGroup(slotTexture, slotBlendMode);
...
(function (spine) {
    ...
    (function (threejs) {
        var ThreeJsTexture = (function (_super) {
            ...
            ThreeJsTexture.toThreeJsBlending = function (blend) {
                if (blend === spine.BlendMode.Normal)
                    return THREE.NormalBlending;
                else if (blend === spine.BlendMode.Additive)
                    return THREE.AdditiveBlending;
                else if (blend === spine.BlendMode.Multiply)
                    return THREE.MultiplyBlending;
                else if (blend === spine.BlendMode.Screen)
                    throw new Error("BlendMode Screen not supported by Three.js");
                else
